...
void SetDefaultBrowser(wchar_t * lpValueName)
{
	...
	if (hr >= 0 && pAAR)
	{
		...
		if (!ifA){
			...
			if (!FAILED(hr)) {
				hr = (*(int(__stdcall **)(PVOID, PVOID))(*(DWORD *)pAAR3 + 24))(pAAR3, lpValueName);
				if (!FAILED(hr)) {
					ifA = true;
				}
				hr = (*(int(__stdcall **)(PVOID, DWORD, PVOID, PVOID))(*(DWORD *)pAAR + 20))(pAAR, 1, lpValueName, &hRet);
				if (pAAR3)
		...
		if (!ifA) {
			...
			if (!FAILED(hr)) {
				hr = (*(int(__stdcall **)(PVOID, PVOID))(*(DWORD *)pAAR4 + 20))(pAAR4, lpValueName);
				if (!FAILED(hr)) {
					ifA = true;
				}
				hr = (*(int(__stdcall **)(PVOID, DWORD, PVOID, PVOID))(*(DWORD *)pAAR + 20))(pAAR, 1, lpValueName, &hRet);
